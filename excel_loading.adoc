...
===== 업로드 모드 =====
데이터 업로드 모드에는 세가지가 존재한다. 

image::images/upload_mode.gif[]

기본모드:: 기본 모드는 현재 있는 매출 데이터를 삭제하고 다시 업로드하는 모드이다. 

업데이트 모드 :: 업데이트 모드는 현재 있는 매출 데이터를 삭제하지 않고,  기존 매출 데이터에서 in_amt, in_qty, out_amt, out_qty, stock_amt, stock_qty 항목에 대하여 업데이트가 수행되는 모드이다. 

Summary 모드:: Summary 모드가 활성화 되어 있으면, 업로드 하려는 엑셀 매출데이터에서 중복되는 reference 가 존재하는 경우 in_amt, in_qty, out_amt, out_qty, stock_amt, stock_qty 항목에 대하여 합을 구한 후 업로드하는 모드이다. 
